...
class RecursiveCascadedNetworks(Network):
    ...
    def build(self, imgT1_fixed,imgT1_float, seg1, seg2, pt1, pt2, pseudo_label):
        ...
        if self.scheme == 'seg':
            ...
            sigma = mean**2/tf.log(2.0)
            confidence_map = tf.exp(-error**2/sigma)
            ret['confidence_map'] = confidence_map
            ...
            #WIST
            warped_T1_s2t_2 = WIST(warped_T1,imgT1_fixed,10,confidence_map)
            warped_T1_s2t = warped_T1_s2t_2 * tf.where(warped_T1>0,tf.ones_like(warped_T1),tf.zeros_like(warped_T1))
            ret['weighted_ist'] = warped_T1_s2t*255
            ...
            #loss    
            ret['target_loss'] = combo_loss()(confidence_map*pseudo_label/255.0,confidence_map*seg_result)
